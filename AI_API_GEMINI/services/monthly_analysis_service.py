...
위 전체의 꿈과 운세를 종합 분석하여, 감동적이고 서정적인 월간 편지를 작성해주세요.
사용자에게 보내는 편지 형식으로 작성해주세요. 

### 구성
1. **인사말** (50자): 해당 월의 날씨/계절 특징을 은유적으로
2. **이번 달의 감정 여정** (200-300자): 초중후반의 감정 흐름을 이야기체로
3. **꿈이 전했던 메시지** (300-400자): 꿈 내용들을 토대로 사용자에게 문학적으로 제시
4. **심리적 통찰과 성장** (200-300자): 내면의 변화와 성장
5. **다음 달을 위한 조언** (100-150자): 따뜻하고 실천 가능한 격려
6. **마무리 말** (70자) : 사용자의 마음을 관통하여 감동을 주는 편지 마무리.
### 작성 가이드
- 사용자 이름을 자연스럽게 사용
- 감동적이고 개인화된 느낌
- 심리학적 깊이 + 문학적 아름다움
- 통계 수치는 자연스럽게 녹여서 딱 한번만 언급해주세요.
- 각 섹션 구분을 나누지 말고 단락만 나눠서 흐름을 자연스럽게하여 사용자에게 하나의 글로 작성해주세요.
- 특정 날짜를 언급하지 마.
...
async def generate_monthly_report_with_gemini(prompt: str, stats: dict) -> str:
    """
    Gemini API를 사용하여 월간 리포트 생성
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="Gemini API 요청 시간 초과")
...
async def process_monthly_analysis(request: MonthlyAnalysisRequest) -> MonthlyAnalysisResponse:
    ...
    try:
        ...
        # 2. Gemini API로 월간 리포트 생성
        monthly_report = await generate_monthly_report_with_gemini(prompt, stats)
